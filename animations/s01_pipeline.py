...
def survey_card(rows=4, w=1.38, rh=0.25, seed=7):
    ...
    for i, f in enumerate(fields[:rows]):
        ...
        bg  = Rectangle(width=w, height=rh, fill_color="#F0FFF0", fill_opacity=1.0,
...
class Pipeline(Scene):
    def construct(self):                                   # noqa: C901
        ...
        # X nodes (survey): same size, labeled Z₉–Z₁₃
        N_X   = 5
        X_R   = 0.22
        X_y_c = -1.65
        X_stp = 0.40
        X_ys  = [X_y_c + (2.0 - i) * X_stp for i in range(N_X)]
        X_lbls = ["Z₉","Z₁₀","Z₁₁","Z₁₂","Z₁₃"]
        ...
        # dense → SAE (14 nodes — more than Prithvi-EO's 10)
        N_SAE    = 14
        N_ACTIVE = 8
        sparse = dot_col(N_SAE, PURPLE_LIGHT, h=2.80).move_to([X2, Y_S, 0])
        a2     = make_arrow(dense.get_right(), sparse.get_left() + LEFT * 0.04,
                            color=PURPLE_LIGHT)
        lbl2   = Text("SAE", color=PURPLE_LIGHT).scale(0.30)
        ...
        # 8 neurons activate (bright + larger), 6 dim out
        ...
        srv_lbl = Text("survey", color=GREEN_LIGHT).scale(LABEL_SCALE)
        srv_lbl.next_to(card, DOWN, buff=0.12)

        self.play(FadeIn(card), FadeIn(srv_lbl), run_time=0.70)
        self.wait(0.25)

        sfeat = dot_col(5, GREEN_LIGHT, h=1.00).move_to([X2, Y_V, 0])
        ...
        # ══════════════════════════════════════════════════════════════════
        Z_nodes = [
            cand_node(np.array([X2, yy, 0.0]), PURPLE_LIGHT, lbl, r=Z_R, sc=0.35)
            for yy, lbl in zip(Z_ys, Z_lbls)
        ]
        X_nodes = [
            cand_node(np.array([X2, yy, 0.0]), GREEN_LIGHT, lbl, r=X_R, sc=0.32)
            for yy, lbl in zip(X_ys, X_lbls)
        ]

        # Pre-build faint arrows from every node to Y (shown in ACT 3)
        ...
        #         T, Y, and light Z→Y arrows appear together
        # ══════════════════════════════════════════════════════════════════
        self.play(
        ...
        T_node = cand_node(T_pos, BLUE_LIGHT,  "T", r=TY_R, sc=0.48)
        Y_node = cand_node(Y_pos, GREEN_LIGHT, "Y", r=TY_R, sc=0.48)
        ...
        # ══════════════════════════════════════════════════════════════════
        # ACT 4 — NEXIS algorithm: test arrows one by one, select 3
        # ══════════════════════════════════════════════════════════════════
        nexis_lbl = Text("NEXIS", color=YELLOW_LIGHT).scale(0.46)
        nexis_lbl.to_edge(UP, buff=0.35)
        self.play(FadeIn(nexis_lbl, shift=DOWN * 0.10), run_time=0.55)
        self.wait(0.25)
        ...
        sel_sim = []
        scan_rounds = [
            (1, [0, 3]),      # scan Z₁, Z₄  →  add Z₂
            (4, [2, 5]),      # scan Z₃, Z₆  →  add Z₅
            (6, [0, 7]),      # scan Z₁, Z₈  →  add Z₇
        ]

        for target_idx, scan_idxs in scan_rounds:
            for s_idx in scan_idxs:
                if s_idx not in sel_sim:
                    self.play(
            ...
            self.play(
                Z_nodes[target_idx].animate.set_opacity(1.0).set_color(TEAL_LIGHT),
                all_ZtoY[target_idx].animate.set_opacity(1.0).set_color(TEAL_LIGHT),
                run_time=0.38,
            )
            sel_sim.append(target_idx)
            self.wait(0.32)

        # Backward check: pulse selected nodes (visual only)
        for idx in sorted(sel_sim):
        ...
        non_sel_arrows = (
            [all_ZtoY[i] for i in range(N_Z) if i not in SEL_Z]
            + all_ZtoY[N_Z:]
        )

        self.play(
            *[n.animate.set_opacity(0.07) for n in non_sel_Z + X_nodes],
            *[a.animate.set_opacity(0.03) for a in non_sel_arrows],
            run_time=0.65,
        )
        self.wait(0.25)
