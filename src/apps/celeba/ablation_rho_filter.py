...
x 4 sweeps, 2,100 runs per dictionary); src/apps/celeba/run_statistics.py aggregates the
variant "rho05_filter" into the paper numbers.  That variant is
...
CPU only (the SLURM wrapper requests 12 cores, 160 GB, 12 h per job); existing CSVs are skipped.

    bash   scripts/celeba/submit_rho_filter.sh             # submit all 12 SLURM jobs
    python src/apps/celeba/ablation_rho_filter.py <tree> <sweep> <fixed> \
           [--seeds N] [--params p1,p2] [--out F]           # e.g. k5/sae n 5
    python src/apps/celeba/run_statistics.py               # the paper numbers
"""
import argparse
import json
import os
