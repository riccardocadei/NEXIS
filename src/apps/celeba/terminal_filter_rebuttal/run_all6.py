...
def one(param, seed):
    ...
    try:
        ...
        return []
    y, t, Z = d.Y, d.T, d.Z
    r_std = nexis(y=y, t=t, z=Z, alpha=ALPHA, max_rounds=MAX_STEPS)
    S = list(r_std.selected)
    o2 = certify(y, t, Z, S, M)
    r_pw = nexis(y=y, t=t, z=Z, alpha=ALPHA, max_rounds=MAX_STEPS,
                 backward_gate="pathwise")
    S_pw = list(r_pw.selected)
    ...
    if len(cols):
        ...
        r_pw_s = nexis(y=y, t=t, z=Zs, alpha=ALPHA, max_rounds=MAX_STEPS,
                       backward_gate="pathwise")
        s_pw = [int(cols[j]) for j in r_pw_s.selected]
    else:
        s_sel, s_o2, s_pw = [], [], []
        r_pw_s = None
    ...
    def bwd_stats(res, truth_set):
        if res is None:
            return {}
        log = res.metadata.get("backward_log", [])
        rem = [e for e in log if not e["retained"]]
        return {"bwd_tests": len(log), "bwd_removed": len(rem),
                "bwd_removed_true": sum(1 for e in rem if int(e["j"]) in truth_set),
                "bwd_removed_false": sum(1 for e in rem if int(e["j"]) not in truth_set)}

    ts = set(TRUTH)
    ts_scr = set(int(np.where(cols == j)[0][0]) for j in TRUTH
                 if len(cols) and j in set(cols.tolist()))
    rows = []
    for v, sel, res, tset in [("base", S, r_std, ts), ("opt2", o2, None, ts),
                              ("pathwise", S_pw, r_pw, ts),
                              ("scr", s_sel, None, ts), ("scr_opt2", s_o2, None, ts),
                              ("scr_pathwise", s_pw, r_pw_s, ts_scr)]:
        rows.append({**tag, "variant": v, **score(sel), **bwd_stats(res, tset)})
    return rows
...
df.to_csv(HERE / f"all6_{tag}.csv", index=False)
print(f"wrote {len(df)} rows -> all6_{tag}.csv")
