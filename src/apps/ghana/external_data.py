...
Every external source is a candidate NEXIS Z effect modifier, keyed by `comm`
— one row per community. A covariate only needs to be *excluded* from Z if
...
download_rainfall.py) is the first source.
"""
...
def load_effect_modifiers(data_dir: Path | str = DATA_DIR) -> pd.DataFrame:
    """Community-level candidate NEXIS Z moderators.
    ...
    if climatology_path.exists() and annual_path.exists() and cdd_path.exists():
        ...
        merged = climatology.merge(annual_wide, on='comm').merge(cdd, on='comm')
        return merged[['comm', *out_columns]]
