...
|---|---|---|---|---|---|---|---|---|
| [LEAP-1000 household survey](#leap-1000-household-survey-core-restricted) | UNICEF Ghana (proprietary, not public) | 83 KB | Tabular survey | Household | 30 | 24 = 20 raw + 4 derived | 2015 only (baseline — covariates must be pre-treatment; only `Y` itself uses both 2015 and 2017) | ✅ |
| ↳ | | | | Community | | 2 derived | Static (time-invariant per community — the "pre-treatment" question doesn't apply) | ✅ |
| [Satellite imagery](#satellite-imagery-landsat-8-via-google-earth-engine) | [Landsat 8](https://developers.google.com/earth-engine/datasets/catalog/LANDSAT_LC08_C02_T1_L2) (USGS/NASA), via Google Earth Engine | ~7 GB | Imagery (6-band) | Community | 6 bands/tile (166×166 px) | 143 (131 neurons representation + 12 spectral indices) | 2015 only (baseline, by design) | ✅ |
| [Rainfall / drought exposure](#rainfall--drought-exposure-chirps-via-google-earth-engine) | [CHIRPS](https://developers.google.com/earth-engine/datasets/catalog/UCSB-CHG_CHIRPS_DAILY) (UCSB Climate Hazards Center), via Google Earth Engine | <1 MB | Tabular climate | Community | 1 (daily precip., aggregated to annual) | 3 (average, volatility rainfall, drought frequency) | 2000–2014 only (pre-baseline climatology) | ✅ |
| ↳ | | | | Community | | 2 (annual rainfall) | 2015 and 2017 (of 3 retrieved — 2016 has no matching survey wave, so it's downloaded but unused; deliberately wave-varying, unlike every row above, since this is a DiD control, not a covariate) | ✅ |
...
**Note on 2016**: `rainfall_annual.csv` has all 3 study years (2015/2016/2017) — 2016 was downloaded to match the LEAP 1000 report's own year-by-year shocks table (Table 4.2.7 reports 2015/2016/2017 separately). But `data.py::load_data()` only merges rainfall for years with a matching survey wave (wave 0 → 2015, wave 1 → 2017); there is no 2016 survey wave, so 2016's row sits in the CSV unused by the current analysis. Kept rather than dropped in case a future UNICEF data drop adds a midline wave.
