...
| ↳ | | | | Community | 1,096 daily precipitation values | 3 (annual rainfall) | 2015–2017 | ✅ |
...
**Baseline-only by design**: every covariate/moderator row above is either fixed at 2015 or genuinely time-invariant — this is deliberate, not incidental. A cash transfer can plausibly change things like business ownership, livestock, or housing by 2017, so using their *endline* values as covariates or effect modifiers would condition on something partly caused by treatment (post-treatment bias). Only two things are allowed to vary with wave/year: `Y` itself (the outcome being differenced, `Y_2017 − Y_2015`), and 2 of the 3 rainfall DiD-control columns (a deliberate exception — they exist specifically to check the estimate against a realized, wave-varying weather shock, not to explain heterogeneity, so the same rule doesn't apply to them). All 3 realized study years (2015/2016/2017) are used: 2015 and 2017 attach directly to the matching survey wave, and 2016 — which has no matching wave to attach to — is folded into the third control (`rainfall_anomaly_2016`, constant across waves) instead of sitting unused. "Raw columns" is the source's native dimensionality (survey columns, imagery bands, the one CHIRPS variable); "Covariates extracted" is how many end up in the analysis after processing, broken into raw-carried-through vs. derived/engineered where that's meaningful. Only satellite imagery needs a representation-learning step (foundation-model embedding + sparse autoencoder) to go from raw to extracted — every other row is already numeric per community and plugs in directly. See per-source sections below for the full detail, and [Extra](#extra-administrative-boundaries--basemaps-plotting-only) for the non-analytical cartographic files (region/district-level boundaries, used only for map plotting).
...
| **Covariates extracted** | 6: 3 community-level `Z` (`rainfall_mean_pre2015`, `rainfall_std_pre2015`, `drought_freq_pre2015`) + 3 DiD control (`rainfall_mm`, `rainfall_anomaly` — per study year, 2015/2017 only — and `rainfall_anomaly_2016`, the 2016 value alone, constant across waves). |
...
| `rainfall/rainfall_annual.csv` | 2015–2017 (study window) | 3 rows × 2 cols: `rainfall_mm`, `rainfall_anomaly` | Realized annual rainfall + anomaly vs. the same climatology → `analysis.py::regression_did(controls=[...])` robustness check, **not** a NEXIS moderator. `rainfall_mm`/`rainfall_anomaly` attach directly for 2015 (wave 0) and 2017 (wave 1); `external_data.py::load_did_controls` additionally derives `rainfall_anomaly_2016` (the 2016 value, broadcast to all 3 rows per `comm`) so 2016 — which has no matching survey wave — still gets used, as a third, wave-invariant control rather than sitting unused. |

Example (community 14, Garu-Tempane): mean 945mm/yr, std 111mm, drought in 2/15 pre-2015 years (13.3%); realized 2015/2016/2017 anomalies of +0.62σ / −0.52σ / −0.77σ against that same baseline, so `rainfall_anomaly_2016` = −0.52σ for this community.

**Note on 2016**: `rainfall_annual.csv` has all 3 study years (2015/2016/2017) — 2016 was downloaded to match the LEAP 1000 report's own year-by-year shocks table (Table 4.2.7 reports 2015/2016/2017 separately). `data.py::load_data()` merges `rainfall_mm`/`rainfall_anomaly` only for years with a matching survey wave (wave 0 → 2015, wave 1 → 2017) — there is no 2016 survey wave, so those two columns' 2016 row is never picked up. `rainfall_anomaly_2016` is how 2016 still enters the analysis: the 2016 value alone (not averaged with 2015/2017, which would mostly re-encode signal the per-wave column already contributes — the three realized years are anti-correlated with each other, e.g. 2016 vs. 2015 is r=−0.66), broadcast onto every household regardless of wave. 2016 is also substantively motivated on its own: it's the growing season immediately before the 2017 endline, so a bad 2016 rainy season can plausibly depress 2017 harvests/consumption directly.
