...
| [Rainfall / drought exposure](#rainfall--drought-exposure-chirps-via-google-earth-engine) | [CHIRPS](https://developers.google.com/earth-engine/datasets/catalog/UCSB-CHG_CHIRPS_DAILY) (UCSB Climate Hazards Center), via Google Earth Engine | <1 MB | Tabular climate | Community | 162 communities × 6,575 daily precip. values (5,479 days 2000–2014 + 1,096 days 2015–2017), aggregated to annual | 3 (average, volatility rainfall, drought frequency) | 2000–2014 | ✅ |
...
| **Covariates extracted** | 6: 3 community-level `Z` (`rainfall_mean_pre2015`, `rainfall_std_pre2015`, `drought_freq_pre2015`) + 3 DiD control (`rainfall_mm`, `rainfall_anomaly` — per study year, 2015/2017 only — and `rainfall_anomaly_1517_mean`, the mean over all 3 realized years 2015/2016/2017, constant across waves). |
...
| `rainfall/rainfall_annual.csv` | 2015–2017 (study window) | 3 rows × 2 cols: `rainfall_mm`, `rainfall_anomaly` | Realized annual rainfall + anomaly vs. the same climatology → `analysis.py::regression_did(controls=[...])` robustness check, **not** a NEXIS moderator. `rainfall_mm`/`rainfall_anomaly` attach directly for 2015 (wave 0) and 2017 (wave 1); `external_data.py::load_did_controls` additionally derives `rainfall_anomaly_1517_mean` (mean over all 3 rows per `comm`) so 2016 — which has no matching survey wave — still gets used, as a third, wave-invariant control rather than sitting unused. |

Example (community 14, Garu-Tempane): mean 945mm/yr, std 111mm, drought in 2/15 pre-2015 years (13.3%); realized 2015/2016/2017 anomalies of +0.62σ / −0.52σ / −0.77σ against that same baseline, giving `rainfall_anomaly_1517_mean` = −0.22σ.

**Note on 2016**: `rainfall_annual.csv` has all 3 study years (2015/2016/2017) — 2016 was downloaded to match the LEAP 1000 report's own year-by-year shocks table (Table 4.2.7 reports 2015/2016/2017 separately). `data.py::load_data()` merges `rainfall_mm`/`rainfall_anomaly` only for years with a matching survey wave (wave 0 → 2015, wave 1 → 2017) — there is no 2016 survey wave, so those two columns' 2016 row is never picked up. `rainfall_anomaly_1517_mean` is how 2016 still enters the analysis: it's computed across all 3 rows per `comm` and merges onto every household regardless of wave.
