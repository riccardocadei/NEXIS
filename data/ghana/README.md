...
|---|---|---|---|---|---|---|---|---|
| [LEAP-1000 household survey — household-level](#leap-1000-household-survey-core-restricted) | UNICEF Ghana (proprietary, not public) | Tabular survey | Household | 20 | 24 = 20 raw + 4 derived | 2015, 2017 | 83 KB | ✅ |
| [LEAP-1000 household survey — community-level](#leap-1000-household-survey-core-restricted) | UNICEF Ghana (same file, derived from GPS/`hhid`) | Tabular survey | Community | 0 | 2 = 0 raw + 2 derived | 2015, 2017 | (same file) | ✅ |
...
| [Rainfall — pre-treatment climatology](#rainfall--drought-exposure-chirps-via-google-earth-engine) | [CHIRPS](https://developers.google.com/earth-engine/datasets/catalog/UCSB-CHG_CHIRPS_DAILY) (UCSB Climate Hazards Center), via Google Earth Engine | Tabular climate | Community | 1 (daily precip., aggregated to annual) | 3 (typical rainfall level, year-to-year volatility, historical drought frequency) — candidate treatment-effect modifiers | 2000–2014 (pre-baseline) | <1 MB | ✅ |
| [Rainfall — realized annual shocks](#rainfall--drought-exposure-chirps-via-google-earth-engine) | CHIRPS, same as above | Tabular climate | Community | 1 | 2 (that year's rainfall total, and how far it was from the community's own historical average) — used only as an optional check that the treatment-effect estimate isn't confounded by an uneven weather shock, not searched as a modifier | 2015–2017 (study window) | (same file set) | ✅ |
...
"Level" is the unit each row's covariates are natively measured at — household, community, or (for some planned sources) district — before anything is merged onto the household panel (`comm` is the join key for every community-level row; no source here operates at an individual-within-household or region level today). The household survey and the rainfall data each get **two rows**: the survey because it contributes covariates at two different levels (most questions are per-household, but 2 are derived down to per-community), and rainfall because its 5 columns serve two genuinely different purposes, not just two levels of one thing — see the "Covariates extracted" description in each row, and the deep-dive section below, for exactly what that distinction means and why it matters. "Raw columns" is the source's native dimensionality (survey columns, imagery bands, the one CHIRPS variable) *for that row*; "Covariates extracted" is how many end up in the analysis after processing, broken into raw-carried-through vs. derived/engineered where that's meaningful. Only satellite imagery needs a representation-learning step (foundation-model embedding + sparse autoencoder) to go from raw to extracted — every other row is already numeric per community and plugs in directly. See per-source sections below for the full detail, and [Extra](#extra-administrative-boundaries--basemaps-plotting-only) for the non-analytical cartographic files (region/district-level boundaries, used only for map plotting).
