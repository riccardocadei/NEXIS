...
| [Market prices](#market-prices-wfp-food-prices-2014-2015) | [WFP Food Prices](https://data.humdata.org/dataset/wfp-food-prices-for-ghana) (via HDX) | 91 markets, 32k+ price obs. | Tabular time series | Community + Regional | 16 (per price record) | 2 in `W` (distance + maize price) + 1 extracted but not in `W` (2017 price, for expenditure deflation) | 2014–2015 (+2016–2017) | ✅ |
...
`regional` is the odd one out, and exists for exactly one covariate so far: `maize_price_2015`. Unlike a distance, a market's mean price is the *same* value for every community assigned to that market — with only 9 markets covering all 162 communities (cluster sizes 3–35, one market post-2015 serving 109/162), tagging this `community` would overstate how granular the variation actually is. `regional` means "shared by a market-catchment-sized cluster of communities, coarser than community, not aligned to administrative districts" (market catchments are geographic, not district-bounded). `district` remains unused today but reserved for sources that are genuinely district-administrative (e.g. EM-DAT, currently rejected — see below).

## Timing

A `timing` tag (`src/apps/covariates.py::Timing`) records a covariate's relationship to the treatment window — `pre` (measured before/at baseline — the overwhelming majority, and the only timing NEXIS's `W` is built from by default), `during` (measured within the treatment window, e.g. `rainfall_2016`/`rainfall_2017`/`max_dry_days_2015_2017`, and the ACLED 2015-2017 columns — all exogenous regardless of timing, so they clear the same bar as rainfall's study-window rows), or `post` (measured after endline — not registered as a NEXIS covariate at all, since a post-treatment value risks collider bias; `maize_price_2017` is the only example so far, extracted for expenditure deflation, not as an effect modifier).
...
| **Covariates extracted** | 2 feed NEXIS's `W`: `dist_nearest_market_km` (`level=community`, distance to the nearest market with a Maize price observation, always defined, range 1–52km, median 14km) and `maize_price_2015` (`level=regional`, that market's mean Maize price over 2014-2015, GHS, range 68–114 across 9 distinct nearest-markets). A third column, `maize_price_2017`, is written to the same CSV but deliberately **not** loaded into `W` — see below. |
| **Status** | ✅ complete. |
...
**2014 isn't optional pooling for stability — it's load-bearing**: restricted to 2015 alone, 7 of the 9 markets currently assigned as someone's nearest-market have *zero* Maize observations that year (their only observation is from 2014). Median distance to the nearest priced market jumps from 13.7km to 75.3km, and only 2 distinct markets would cover all 162 communities instead of 9 — not a marginal loss, closer to a degenerate covariate. Keep the pool.

**Why 2014-2015, not later years, for the `W` covariate**: deliberately pre/at-baseline, not concurrent with or after treatment — sidesteps the classic "cash transfer causes local price inflation" general-equilibrium debate entirely (a household-level cash transfer cannot retroactively change a 2014-2015 price), same logic as rainfall's pre-2015 climatology. Since food prices are genuinely dated historical records (like ACLED), there's no need to use present-day data the way OpenCellID/Ookla were forced to.

**Why nearest-market assignment rather than a district-level price**: markets are point locations, not administrative units, so assigning each community its geographically nearest priced market (rather than "the market in this community's district") gives more granular variation — 9 distinct nearest-markets across the 162 communities, not a 5-category district dummy. That granularity is still coarser than "community", though: with only 9 markets serving 162 communities (cluster sizes 3–35), `maize_price_2015` is a genuine step function shared across a market's catchment area, not per-community variation the way `dist_nearest_market_km`'s continuous haversine distance is — hence `level=regional`, not `community` (see [Level](#level) below).

**`maize_price_2017` (2016-2017 pooled, `timing=post`) — not a `W` covariate**: extracted for deflating/actualizing expenditures post-endline (a different use case than an effect modifier), not registered in `data.py`'s `COVARIATES`. Using a post-treatment price as a "pre-treatment" covariate would risk exactly the collider bias every household-survey exclusion in this project already avoids (see `external_data.py::load_effect_modifiers`'s module docstring) — a cash transfer plausibly *can* move local Maize prices by 2017, unlike a 2014-2015 price it postdates. It's also structurally sparser: WFP Maize monitoring near the LEAP districts largely collapsed after 2015 — only 3 distinct nearest-markets (vs. 9 pre-period), median distance 62.1km (vs. 13.7km). Pooling 2016 into the window doesn't recover any closer markets (2016-only, 2016-2017, and 2017-only all resolve to the identical 3 nearest-markets/distances); it only adds more observations to average per market, which is why 2016-2017 is used rather than 2017 alone.

**No genuine "during-treatment" price**: checked whether a third window between pre (2014-2015) and post (2016-2017) exists in the data — it doesn't. 2016-only, 2016-2017, and 2017-only all resolve to the same 3 nearest-markets per community, so there's no additional market that lights up specifically "during" and not "post." `Timing.DURING` remains in the enum for other domains where it does apply (e.g. `rainfall_2016`/`rainfall_2017`, the ACLED 2015-2017 columns — see [Timing](#timing) below) — market prices just don't have a distinct during-window worth extracting.
